...
import org.nure.gas_station.services.implementations.FuelOrderServiceService;
...
public class OperationControllerTest {
    ...
    private MockMvc mvc;
    @MockBean
    private FuelOrderServiceService operationService;
    ...
    public void testQueryOperationsShouldReturnGasStaionPageWithOperations() throws Exception {
        ...
        MvcResult result = mvc.perform(post("/api/operation/query-pages").contentType(MediaType.APPLICATION_JSON).content(map.writeValueAsString(operationsPage)))
                .andExpect(status().isOk())
                .andReturn();
        PageDTO<OperationResponse> response = map.readValue(result.getResponse().getContentAsString(), new TypeReference<PageDTO<OperationResponse>>() {});
